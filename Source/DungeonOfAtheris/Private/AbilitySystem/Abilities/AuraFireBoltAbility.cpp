...
#include "AbilitySystem/Abilities/AuraFireBoltAbility.h"
...
FString UAuraFireBoltAbility::GetDescription(int32 Level)
{
...
FString UAuraFireBoltAbility::GetNextLevelDescription(int32 Level)
{
	const int32 ScaledDamage = Damage.GetValueAtLevel(Level);
	const float ManaCost = FMath::Abs(GetManaCost(Level));
	const float Cooldown = GetCooldown(Level);
...
void UAuraFireBoltAbility::SpawnProjectiles(const FVector& ProjectileTargetLocation, const FGameplayTag& SocketTag,
